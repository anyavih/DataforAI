...
* **`dataai.py`**: The main script for training the **roberta-base** NLI model. It handles the 90-10 stratified test-train split and the fine-tuning process.

## Key Data Files

* **`final_triplets_dataset.json`**: This is the full training set saved as a JSON file generated by `data_creator.py` after GPT-4o processes the filtered climate evidence.
* **`verification_sample.json`**: This is the specific file generated by `seven_five_chunks.py` that contains the curated "Golden Set" (the 75 random samples, with the 25 specific human-authored IDs excluded).
* **`filtered_climate_evidence.csv`**: This contains the 10K chunks that we used as evidence for our NLI model.
